...
type VertexDrag = {vertexId: number; start: CanvasPoint; current: CanvasPoint};
type PendingVertexMove = {
  vertexId: number;
...
let hitmap: DesignHitmap | null = null;
let topologyEditLayer: TopologyEditLayer | null = null;
let selectedRegionIds: Set<number> = new Set();
let sourceImage = new Image();
let dragStart: CanvasPoint | null = null;
let dragCurrent: CanvasPoint | null = null;
let lassoPoints: CanvasPoint[] = [];
let activeVertexDrag: VertexDrag | null = null;
let hoveredVertex: TopologyVertex | null = null;
let pendingVertexMove: PendingVertexMove | null = null;
let selectedEdgeIds: Set<number> = new Set();
let cachedBaseCanvas: HTMLCanvasElement | null = null;
let cachedBaseKey = '';
let canvasZoom = 100;
...
async function cleanupReport() {
  ...
  setStatus(
    `Cleanup report: ${report.readiness} (${report.readiness_score}/100), ${report.region_count} regions, coverage ${report.coverage.valid ? 'valid' : 'invalid'}, ${report.merge_suggestion_count} merge suggestion(s), ${report.jagged_boundary_count} jagged boundary group(s).`,
  );
}
